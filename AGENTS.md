...
本仓库用于部署“企业微信智能机器人 → 本机 Codex CLI”长连接桥接服务。它依赖 Codex 提供的模型能力和有效账号；使用者可以通过 ChatGPT 账号或 OpenAI API Key 登录 Codex，但不需要再部署第二套模型服务或路由模型。用户要求部署、安装、配置或排障时，按本文执行。
...
6. 使用者已亲自确认人设、Codex 模型策略、上下文策略、目录权限和自启动选择；代理不得代选。
...
- 不直接复制其他部署的 `.env` 或人设文件，也不要把示例值当作用户选择。
- 让使用者在交互式终端运行 `npm run setup`。
- 必须由使用者选择：人设、偏题提醒、Codex 首选与备用模型、推理强度、速度、工作目录、附加目录、文件权限、会话是否持久以及是否自启动。
- 模型相关项目可以留空以继承使用者自己的 Codex 配置；不要擅自填入发布者曾使用的模型。
- Bot Secret 应由使用者在不回显的向导输入框中填写，不要求其粘贴到聊天。
- 不要求配置第二套模型服务；Codex 身份验证只通过官方 `codex login` 完成，不把登录凭证写入项目配置。
- 向导只生成本机 `.env` 与 `.runtime/persona-profile.json`。
