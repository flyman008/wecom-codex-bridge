...
`status-service.ps1` 会分别显示保活进程、监督进程、业务进程和企微连接状态，并校验 PID 是否确实属于当前服务，避免系统复用旧 PID 时误报在线。业务进程异常退出时由监督进程重启；监督进程异常退出时由保活进程重新拉起；保活进程异常退出时也会由监督进程补回。保活进程还会检查业务健康接口，连续异常时重建整条服务链。
